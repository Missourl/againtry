    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
          ...
          if len(x) > 0:
              ...
              if mean_reward.any() > self.best_mean_reward:
...
# Train the agent
model = PPO("MlpPolicy", env, ent_coef=0.01, n_steps=20, verbose=1)
...
for i in range(1000,2000):
    ...
    env = MCS(T, K, pmax, W, h, t_sense_distribution, E_sense_distribution, p_sense_distribution,
              ...
              normalized_throughput_distribution, initial_battery_state, average_channel_coeff_per_sensor,
              average_channel_gain, avg_SNR_linear,i)
    obs=np.column_stack((self.initial_battery_state.copy(), self.throughput_distribution[0,self.task_types_distribution[self.j]].copy(), self.required_sensors_per_task_distribution[self.j].copy()))
