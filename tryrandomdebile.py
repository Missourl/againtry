...
for i in range(1,1000):
    ...
    for j in range(1,3000):
        ...
        if tt==0:
            ...
            random.seed()
            #A=np.array([random.random()*0.1,random.random()*0.1,random.random()*0.1,random.random()*0.1,random.random()*0.1,random.random()*0.1,random.random()*0.1,random.random()*0.1])
            A=0 + np.random.uniform(0, 1.0009, 8, replace=False) * (0.1 - 0)
            ...
            r = 0
            rr = 0
            while rr != len(sensors):
                if tau_exec[sensors[rr]] <= task_deadline_distribution[j]:
                    rr = rr + 1
                   # print('i', i)
                    r = r + 1
                    print('r', r)
